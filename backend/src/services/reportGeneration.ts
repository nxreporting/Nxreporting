  ...
  try {
    ...
    data.forEach(item => {
      ...
      const row = [
        ...
        item.file.uploadedAt.toISOString().split('T')[0],
  ...
  try {
    ...
    const detailsData = data.map(item => {
      ...
      return {
        ...
        'Upload Date': item.file.uploadedAt.toISOString().split('T')[0],
        'Extraction Date': item.extractedAt.toISOString().split('T')[0],
        'Status': item.status,
